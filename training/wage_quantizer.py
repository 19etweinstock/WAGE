...
def LinearQuantizeInject(x, weight_bits, error_rate, error_dist, error_vals, logger, excluded_bits, name, k=0, error_map=[]):
    
    minQ = torch.min(x)
    delta = torch.max(x) - torch.min(x)
    y = x.clone()

    stepSizeRatio = 2.**(-weight_bits)
    stepSize = stepSizeRatio*delta.item()
    index = torch.clamp(torch.floor((x-minQ.item())/stepSize), 0, (2.**(weight_bits)-1))

    # print(torch.min(x))
    # print(torch.max(x))
    # print(torch.min(index))
    # print(torch.max(index))

    # torch.set_printoptions(threshold=100_000)
    # print(index)

    # inject errors (RESNET)
    if error_rate != 0:
        log = 0
        if error_rate < 2:
            log = 1
        ...
        #### FOR TERNARY WEIGHTS ####
        index = errors.inject_errors(index, 8, error_dist, error_vals, 
                                        logger, log, excluded_bits, name)
        #############################

        #### FOR BINARY WEIGHTS ####
        # index = errors.inject_errors(index, weight_bits+1, error_dist, error_vals, 
        #                                 logger, log, excluded_bits, name)
        #############################        
    ...
    # if error_rate > 2:
        # Shuffle tensor
        # error_map=error_map[torch.randperm(error_map.size()[1])]
        # index = torch.add(index, error_map[k])
        # index = torch.clamp(index, 0, 2**bit-1)                         
...
if __name__ == "__main__":
    ...
    print("Weights")
    print("="*80)
    quant_data = QW(test_tensor, bits_W, 16).data.numpy()
    print(quant_data)
    # test QA
    print("="*80)
    print("Activation")
